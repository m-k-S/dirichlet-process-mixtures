...
def compute_expectations(data, means, precisions_pd, concentrations):
    ...
    with np.errstate(under='ignore'):
        log_likelihoods = log_weighted_prob - \
            softmax_log_weighted_prob[:, np.newaxis]
...
def elbo(log_likelihoods, mean_lwp, precisions, weight_concentrations):
    ...
    log_norm_weight = - \
        np.sum(betaln(weight_concentrations[0], weight_concentrations[1]))
    return (-np.sum(np.exp(log_likelihoods) * log_likelihoods) -
            log_norm - log_norm_weight - 0.5 * D * np.sum(np.log(mean_prec)))
...
def dpmm(data, hyperparameters):
    ...
    for i in range(1, n_iter + 1):
        mean_lwp, log_likelihoods = compute_expectations(
            data, means, precisions, concentrations)
    ...
    log_likelihoods = final_truncation(
        data,
        final_means,
        final_precisions,
        final_concentrations,
