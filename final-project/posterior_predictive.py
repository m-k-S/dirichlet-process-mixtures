...
def log_posterior_predictive_eval(x_new, kappa, tau_0, tau_1, S):
    ...
    for s in range(S):
      ...
      for t in range(post_pred_clusters.shape[0]):
        log_prob -= torch.exp(Poisson(post_pred_clusters[t]).log_prob(x_new))
    log_prob /= S
    return log_prob
...
def sq_log_posterior_predictive_eval(x_new, kappa, tau_0, tau_1, S):
    ...
    for s in range(S):
      ...
      post_pred_clusters = lambda_mc[s]
      for t in range(post_pred_clusters.shape[0]):
        log_prob -= torch.exp(Poisson(post_pred_clusters[t]).log_prob(x_new)) ** 2
    log_prob /= S
    return log_prob
...
def posterior_predictive_sample(kappa, tau_0, tau_1, S, M):
    ...
    for s in range(S):
      post_pred_weights = mix_weights(beta_mc[s])
      post_pred_clusters = lambda_mc[s]
      hallucinated_samples[s, :] = MixtureSameFamily(Categorical(post_pred_weights), Poisson(post_pred_clusters)).sample([M])
